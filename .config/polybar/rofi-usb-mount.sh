...
usbcheck(){ \
    mounteddrives="$(lsblk -rpo "name,type,size,mountpoint" | awk '$2=="part"&&$4!=""{printf "%s (%s)\t  ",$1,$3}')"
...
umountall(){ \
    for chosen in $(echo $(lsblk -rpo "name,type,size,mountpoint" | awk '$2=="part"&&$4!=""{printf "%s\n",$1}')); do
...
mounteddrives="$(lsblk -rpo "name,type,size,mountpoint" | awk '$2=="part"&&$4!=""{printf "%s (%s)\n",$1,$3}')"
